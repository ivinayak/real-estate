export class AppContent {
    appContent: any = {
        homePage: {
            topVideo:{
                path: '../assets/videos/home.mp4',
                type: 'video/mp4'
            },
            headerText: 'Real Estate',
            currencySign: '$',
            odometer1: {
                value: 10,
                fraction: 5,
                label: 'Portfolio Value',
                unit: 'BILLION'
            },
            odometer2: {
                value: 56,
                fraction: 25,
                label: 'Sq. ft. of portfolio',
                unit: 'BILLION'
            },
            odometer3: {
                value: 140,
                fraction: 0,
                label: 'Sq. ft. in development',
                unit: 'BILLION'
            },
            introText: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.',
            secondVideo: {
                path: '../assets/videos/home2.mp4',
                type: 'video/mp4'
            },
            image1:{
                path: '../assets/images/img1.jpg'
            },
            image2:{
                path: '../assets/images/img2.jpg',
                label: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit'
            },
            image3:{
                path: '../assets/images/img3.jpg',
                label: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit'
            },
            homeQuote:{
                text: '"I am the hope of the universe. I am the answer to all living things that cry out for peace. I am the protector of the innocent. I am the light in the darkness. I am the truth. Ally to good! Nightmare to you!"',
                from: 'Son Goku, DBZ'
            }
        },
        company:{
            pictureText: 'Company',
            mainPhoto: {
                path: '../assets/images/img5.jpg'
            },
            mainQuote:{
                text: '"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."',
                from: 'Tommy Crooz'
            },
            aboutCompanyTextsLeftColumn: [
                'Lorem ipsum dolor sit amet. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
                'Lorem ipsum dolor sit amet. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
                'Lorem ipsum dolor sit amet. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
            ],
            aboutCompanyTextsRightColumn: [
                'Lorem ipsum dolor sit amet. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
                'Lorem ipsum dolor sit amet. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
                'Lorem ipsum dolor sit amet. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.',
            ],
            image2:{
                path: '../assets/images/img6.jpg'
            },
            companyOps: [
                'Lorem ipsum',
                'Lorem ipsum',
                'Lorem ipsum',
                'Lorem ipsum',
                'Lorem ipsum',
                'Lorem ipsum',
                'Lorem ipsum',
                'Lorem ipsum',
            ],
            image3: {
                path: '../assets/images/img7.jpg'
            },
            principleHeader: 'Guiding Principles',
            principles: [
                {
                    name: 'Experience',
                    about: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                },
                {
                    name: 'Experience',
                    about: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                },
                {
                    name: 'Experience',
                    about: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                },
                {
                    name: 'Experience',
                    about: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                },
            ],
            image4:{
                path: '../assets/images/img6.jpg'
            },
            person:{
                name: 'Son Goku',
                post: 'Super Saiyan',
                quotes: [
                    'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                    'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                    'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                ]
            },
            teamHeader: 'Team',
            team: [
                {
                    imagePath: '../assets/images/img8.png',
                    name: 'Ranuto Muzuaki',
                    post: 'Hokage'
                },
                {
                    imagePath: '../assets/images/img8.png',
                    name: 'Ranuto Muzuaki',
                    post: 'Hokage'
                },
                {
                    imagePath: '../assets/images/img8.png',
                    name: 'Ranuto Muzuaki',
                    post: 'Hokage'
                },
                {
                    imagePath: '../assets/images/img8.png',
                    name: 'Ranuto Muzuaki',
                    post: 'Hokage'
                },
                {
                    imagePath: '../assets/images/img8.png',
                    name: 'Ranuto Muzuaki',
                    post: 'Hokage'
                },
                {
                    imagePath: '../assets/images/img8.png',
                    name: 'Ranuto Muzuaki',
                    post: 'Hokage'
                }
            ]
        },
        investment:{
            mainPhoto: {
                path: '../assets/images/img9.jpg'
            },
            header: 'Investment',
            quote:{
                text: '"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."',
                from: 'Tommy Crooz'
            },
            waysOfInvesting: [
                {
                    header: 'Investing in iconic real estate',
                    text: [
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                    ]
                },
                {
                    header: 'Investing in iconic real estate',
                    text: [
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                    ]
                },
                {
                    header: 'Investing in iconic real estate',
                    text: [
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                    ]
                }
            ],
            benefits:[
                {
                    header: 'Core+',
                    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                    imagePath: '../assets/images/img10.jpg'
                },
                {
                    header: 'Value Add',
                    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                    imagePath: '../assets/images/img11.jpg'
                },
                {
                    header: 'Opportunistic',
                    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                    imagePath: '../assets/images/img12.jpg'
                }
            ],
            financialPartnerHeader: 'Financial Partners',
            partners: [
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
            ]
        },
        collaborators: {
            mainPhoto:{
                path: '../assets/images/img14.png',
                text: 'Collaborators'
            },
            header: 'The Art of Collaboration',
            texts: [
                'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamcolaboris nisi ut aliquip ex ea commodo consequat.',
                'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamcolaboris nisi ut aliquip ex ea commodo consequat.',
                'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamcolaboris nisi ut aliquip ex ea commodo consequat.',
                'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamcolaboris nisi ut aliquip ex ea commodo consequat.'
            ],
            collaboratorHeader: 'The Collaborators',
            collaboratorList: [
                {
                    name: 'Ranuto Muzuaki',
                    imagePath: '../assets/images/img8.png'
                },
                {
                    name: 'Ranuto Muzuaki',
                    imagePath: '../assets/images/img8.png'
                },
                {
                    name: 'Ranuto Muzuaki',
                    imagePath: '../assets/images/img8.png'
                },
                {
                    name: 'Ranuto Muzuaki',
                    imagePath: '../assets/images/img8.png'
                },
                {
                    name: 'Ranuto Muzuaki',
                    imagePath: '../assets/images/img8.png'
                },
                {
                    name: 'Ranuto Muzuaki',
                    imagePath: '../assets/images/img8.png'
                },
                {
                    name: 'Ranuto Muzuaki',
                    imagePath: '../assets/images/img8.png'
                }
            ],
            companies: [
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                },
                {
                    imagePath: '../assets/images/img13.png'
                }
            ]
        },
        portfolio:{
            assets: [
                {
                    pfPageImagePath: '../assets/images/img15.jpg',
                    modalImagePath: '../assets/images/img15.jpg',
                    name: 'Hokage Office',
                    countryCode: 'US',
                    details: [
                        {
                            key: 'Address',
                            value: 'Mount Hokage, 1982, CF'
                        },
                        {
                            key: 'Use',
                            value: 'Office, Retail, Park'
                        },
                        {
                            key: 'Size',
                            value: '200299 Square Feet'
                        },
                        {
                            key: 'Notable Tenants',
                            value: 'Ranuto Muzuaki, Suchiha Aasuke'
                        },
                        {
                            key: 'Architecture',
                            value: 'Edd and Eddy'
                        },
                        {
                            key: 'Interior Design',
                            value: 'Kasura Ruhano'
                        },
                        {
                            key: 'Year Built',
                            value: '1982'
                        },
                        {
                            key: 'Investment Strategy',
                            value: 'Value Add'
                        }
                    ],
                    location: 'America',
                    category: 'Commercial',
                    texts: [
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                    ],
                    imageCarouselPaths: [
                        {
                            imagePath: '../assets/images/img16.jpg'
                        },
                        {
                            imagePath: '../assets/images/img17.jpg'
                        },
                        {
                            imagePath: '../assets/images/img18.jpg'
                        },
                        {
                            imagePath: '../assets/images/img6.jpg'
                        },
                        {
                            imagePath: '../assets/images/img12.jpg'
                        }
                    ],
                    about: [
                        {
                            text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                            imagePath: '../assets/images/img2.jpg'
                        },
                        {
                            text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                            imagePath: '../assets/images/img6.jpg'
                        },
                        {
                            text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                            imagePath: '../assets/images/img16.jpg'
                        },
                        {
                            text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                            imagePath: '../assets/images/img3.jpg'
                        }
                    ],
                    videoCarouselPaths: [
                        {
                            videoPath: '../assets/videos/home.mp4',
                            text: 'Our future',
                            type: 'video/mp4'
                        },
                        {
                            videoPath: '../assets/videos/home2.mp4',
                            text: 'Watch this',
                            type: 'video/mp4'
                        },
                        {
                            videoPath: '../assets/videos/home.mp4',
                            text: 'Building Tour',
                            type: 'video/mp4'
                        },
                        {
                            videoPath: '../assets/videos/home2.mp4',
                            text: 'Dont watch this',
                            type: 'video/mp4'
                        }
                    ]
                },
                {
                    pfPageImagePath: '../assets/images/img15.jpg',
                    modalImagePath: '../assets/images/img15.jpg',
                    name: 'Hokage Office',
                    countryCode: 'US',
                    details: [
                        {
                            key: 'Address',
                            value: 'Mount Hokage, 1982, CF'
                        },
                        {
                            key: 'Use',
                            value: 'Office, Retail, Park'
                        },
                        {
                            key: 'Size',
                            value: '200299 Square Feet'
                        },
                        {
                            key: 'Notable Tenants',
                            value: 'Ranuto Muzuaki, Suchiha Aasuke'
                        },
                        {
                            key: 'Architecture',
                            value: 'Edd and Eddy'
                        },
                        {
                            key: 'Interior Design',
                            value: 'Kasura Ruhano'
                        },
                        {
                            key: 'Year Built',
                            value: '1982'
                        },
                        {
                            key: 'Investment Strategy',
                            value: 'Value Add'
                        }
                    ],
                    location: 'America',
                    category: 'Commercial',
                    texts: [
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                        'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.'
                    ],
                    imageCarouselPaths: [
                        {
                            imagePath: '../assets/images/img16.jpg'
                        },
                        {
                            imagePath: '../assets/images/img17.jpg'
                        },
                        {
                            imagePath: '../assets/images/img18.jpg'
                        },
                        {
                            imagePath: '../assets/images/img6.jpg'
                        },
                        {
                            imagePath: '../assets/images/img12.jpg'
                        }
                    ],
                    about: [
                        {
                            text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                            imagePath: '../assets/images/img2.jpg'
                        },
                        {
                            text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                            imagePath: '../assets/images/img6.jpg'
                        },
                        {
                            text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                            imagePath: '../assets/images/img16.jpg'
                        },
                        {
                            text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.',
                            imagePath: '../assets/images/img3.jpg'
                        }
                    ],
                    videoCarouselPaths: [
                        {
                            videoPath: '../assets/videos/home.mp4',
                            text: 'Our future',
                            type: 'video/mp4'
                        },
                        {
                            videoPath: '../assets/videos/home2.mp4',
                            text: 'Watch this',
                            type: 'video/mp4'
                        },
                        {
                            videoPath: '../assets/videos/home.mp4',
                            text: 'Building Tour',
                            type: 'video/mp4'
                        },
                        {
                            videoPath: '../assets/videos/home2.mp4',
                            text: 'Dont watch this',
                            type: 'video/mp4'
                        }
                    ]
                },
            ]
        },
        world:{
            topLeftImage: {
                path: '../assets/images/img1.jpg'
            },
            topRightImages: [
                {
                    path: '../assets/images/img2.jpg',
                    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit'
                },
                {
                    path: '../assets/images/img3.jpg',
                    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit'
                },
                {
                    path: '../assets/images/img4.jpg',
                    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit'
                },
                {
                    path: '../assets/images/img6.jpg',
                    text: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit'
                },
            ],
            bottomCards: [
                {
                    imagePath: '../assets/images/img18.jpg',
                    videoPath: '../assets/videos/home2.mp4',
                    videoType: 'video/mp4',
                    text: 'Lorem ipsum dolor sit amet'
                },
                {
                    imagePath: '../assets/images/img3.jpg',
                    videoPath: '../assets/videos/home.mp4',
                    videoType: 'video/mp4',
                    text: 'Lorem ipsum dolor sit amet'
                },
                {
                    imagePath: '../assets/images/img7.jpg',
                    videoPath: '../assets/videos/home2.mp4',
                    videoType: 'video/mp4',
                    text: 'Lorem ipsum dolor sit amet'
                },
                {
                    imagePath: '../assets/images/img4.jpg',
                    videoPath: '../assets/videos/home.mp4',
                    videoType: 'video/mp4',
                    text: 'Lorem ipsum dolor sit amet'
                },
                {
                    imagePath: '../assets/images/img5.jpg',
                    videoPath: '../assets/videos/home2.mp4',
                    videoType: 'video/mp4',
                    text: 'Lorem ipsum dolor sit amet'
                },
                {
                    imagePath: '../assets/images/img18.jpg',
                    videoPath: '../assets/videos/home2.mp4',
                    videoType: 'video/mp4',
                    text: 'Lorem ipsum dolor sit amet'
                }
            ]
        }
    }
}